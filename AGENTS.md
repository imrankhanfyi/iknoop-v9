...
> from the **`fork/no-network`** branch, which sits on the v9.0.1 tag and carries **two permanent
...
> `fork/no-network` tracks it and is that repo's default branch, so a plain `git push` lands there).
> So this branch **is** backed up off-machine. It was named `local/no-network` until 2026-07-27, where
> "local" meant *local to this fork* — never "unpushed"; that read as "not backed up" once, hence
