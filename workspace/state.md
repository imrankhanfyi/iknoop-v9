...
_Last updated: 2026-08-14. This is the ongoing log for the active repo. Historical trail
(v1.61–v1.68) is in `~/Projects/NOOP-archive/repo-v1.61/workspace/state.md`._

## NEXT PICKUP — verified BLE recovery + upstream maintenance (2026-08-14)

The pending bounded motion catch-up has now been verified on the real WHOOP 4.0.  On 2026-08-13 the
strap completed normal historical offload and returned to live data: motion/gravity reached 15:12:03,
then HR resumed at 15:12:43.  This ordinary 40-second frontier gap requires no bounded recovery, but
confirms the complete offload-to-live transition.  The persistent local `strapLog.tail` is the source
of this evidence; it is a rolling 2,000-event log in the app container, not a cloud service.

Completed and merged on `fork/no-network`:

1. `17f67c79 fix(ble): retain pending sentinel motion catch-up` — preserves a pending bounded motion
   request when a history response ends at the empty sentinel.  It was release-built, installed, and
   observed during the real 2026-08-13 strap session.
2. `53d293d7 fix(hrv): preserve same-second R-R emission order` — records the strap emission ordinal
   in additive Swift/Room migrations and uses it wherever same-second R-R samples are consumed.
   This prevents RMSSD/HRV from being biased by magnitude sorting.  Swift protocol/store/analytics
   suites and the macOS Release app build passed; Android JVM tests remain unrun because this Mac has
   no Java runtime.  Existing historical R-R rows remain readable with their legacy ordering; newly
   ingested equal-timestamp rows retain their real arrival order.  The Release app from this commit was
   installed at `/Applications/NOOP.app` on 2026-08-14; its sandbox entitlements include Bluetooth but
   no network-client entitlement.

Priority order for the next implementation session:

1. **Reconcile newer BLE empty-offload safeguards — high value, but never cherry-pick blind.** Review
...
2. **Measured sleep-stage transition rule — useful separate follow-up.** Upstream `98d1d1ff` strongly
...
3. **Effort accuracy — separate follow-up if training scores matter.** Upstream `85cc0b4f` weights each
...
4. **Latest-HR SQLite query shape — safe maintenance item, no longer urgent.** Upstream `495f25d3`
...
5. **Optional privacy UX:** upstream v9.3.0 warns that `.noopbak` archives are readable/unencrypted and
